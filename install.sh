...
git clone -b master https://github.com/mmadsen340/ec2-ssh.git

cd "$tmpdir/ec2-ssh"
sudo mv "$tmpdir/ec2-ssh" "$tmpdir/aws-ec2-ssh"

cp authorized_keys_command.sh $AUTHORIZED_KEYS_COMMAND_FILE
sudo chmod 755 $AUTHORIZED_KEYS_COMMAND_FILE
cp import_users.sh $IMPORT_USERS_SCRIPT_FILE
sudo chmod 755 $IMPORT_USERS_SCRIPT_FILE
